/// IoT服务模板代码
class IoTServiceTemplate {
  /// 生成IoT服务代码
  /// [defaultBindings] 默认设备绑定 Map<slotId, {deviceId, secretKey}>
  static String generate({Map<String, Map<String, String>>? defaultBindings}) {
    final bindings = defaultBindings ?? {};
    final bindingCode = bindings.entries
        .where((e) => e.value['deviceId']?.isNotEmpty == true)
        .map((e) => "    bindDevice('${e.key}', '${e.value['deviceId']}', '${e.value['secretKey'] ?? ''}');")
        .join('\n');
